...
var users = sqliteTable("users", {
  ...
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(uniexpoch())`),
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().default(sql`(uniexpoch())`)
});
...
app4.post("/signup", async (c) => {
  ...
  try {
    const [users_instance] = await db.insert(users).values({ id, email: id, passwordHash: hashedPassword }).returning({ id: users.id });

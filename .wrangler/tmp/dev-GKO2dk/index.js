...
// .wrangler/tmp/bundle-9mQMwE/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-9mQMwE/middleware-insertion-facade.js
init_modules_watch_stub();
...
app4.post("/login", async (c) => {
  ...
  try {
    ...
    if (!match2) return c.json({ error: "Invalid password!" });
...
var app5 = new Hono2();
app5.use("/api/*", logger());
app5.route("/api/users", userRoute_default);
app5.route("/api/generalExpenditure", generalExpeditureRoute_default);
app5.route("/api/expenseSummary", expenseSummaryRoute_default);
app5.route("/api/auth", authRoute_default);
app5.use(accessAuth).get("/api/health", (c) => {
  console.log(c.env.ENVIRONMENT);
